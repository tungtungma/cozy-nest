...
// GET /api/admin/stock-analytics/monthly - Get monthly (30-day rolling) stock analytics
export async function GET(request: NextRequest) {
  try {
    ...
    // Parse movement pagination params
    const url = new URL(request.url);
    const movementPage = Math.max(1, parseInt(url.searchParams.get("movementPage") || "1", 10));
    const movementLimit = Math.min(50, Math.max(1, parseInt(url.searchParams.get("movementLimit") || "10", 10)));
    const movementSkip = (movementPage - 1) * movementLimit;

    // Get date range for rolling 30-day window
    const now = new Date();
    const monthStart = subDays(now, 29); // 30-day window including today
    const monthEnd = now;
    ...
    // Get stock movements for the 30-day window with order items to get variant info
    ...
    const getVariantForMovement = (movement: typeof monthMovements[0]) => {
    ...
    const soldThisMonth = monthMovements
      .filter((m: typeof monthMovements[0]) => m.type === "sale")
      .reduce((sum: number, m: typeof monthMovements[0]) => sum + Math.abs(m.quantity), 0);

    const restockedThisMonth = monthMovements
      .filter((m: typeof monthMovements[0]) => m.type === "restock")
      .reduce((sum: number, m: typeof monthMovements[0]) => sum + m.quantity, 0);

    // Group movements by day for chart (with Hong Kong timezone)
    const HONG_KONG_TZ = "Asia/Hong_Kong";
    const dailyMovements = Array.from({ length: 30 }, (_, i) => {
      const day = addDays(monthStart, i); // Day 0 to Day 29
      const dayName = format(day, "MMM dd"); // Jun 01, Jun 02, etc.
      ...
      const dayData = monthMovements.filter((m: typeof monthMovements[0]) => {
    ...
    monthMovements
      .filter((m: typeof monthMovements[0]) => m.type === "sale")
      .forEach((movement: typeof monthMovements[0]) => {
    ...
    const totalMovements = Math.min(monthMovements.length, 50);
    const pagedMovements = monthMovements.slice(movementSkip, movementSkip + movementLimit);
    const recentMovements = await Promise.all(
      pagedMovements.map(async (movement: typeof monthMovements[0]) => {
        const variant = getVariantForMovement(movement);
    ...
    return NextResponse.json({
      ...
      movementPagination: {
        ...
        totalPages: Math.ceil(totalMovements / movementLimit),
        hasNextPage: movementPage < Math.ceil(totalMovements / movementLimit),
        hasPreviousPage: movementPage > 1,
      },
    });
  } catch (error) {
    console.error("Error fetching monthly stock analytics:", error);
    return NextResponse.json(
